...
class AbstractCurses(Generic[ValueT], metaclass=ABCMeta):
...
class SelectMenu(AbstractCurses[ValueT]):
	def __init__(
		self,
		group: MenuItemGroup,
		multi: bool = False,
...
class Tui:
	...
	@staticmethod
	def run(component: AbstractCurses[ValueT]) -> Result[ValueT]:
		if Tui._t is None:
			tui = Tui().init()
			tui.screen.clear()
	...
	def _main_loop(self, component: AbstractCurses[ValueT]) -> Result[ValueT]:
		self._screen.refresh()
